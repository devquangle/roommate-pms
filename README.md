...
- **Bảng phân công nhiệm vụ chi tiết:**
  ...
  | --- | --- | --- |
  | UI/UX & Layout | Thiết kế giao diện SPA, Sidebar, Topbar, Responsive CSS (Bootstrap 5) | Hoàn thành |
  | Core & Business Logic | Xây dựng bộ định tuyến (Router), Logic tính hóa đơn, điện nước, công nợ | Hoàn thành |
  | Storage & Services | Xây dựng dịch vụ CRUD với LocalStorage, Import/Export JSON, Seed Data | Hoàn thành |
  | Testing | Viết Unit/Integration Tests (Vitest) và E2E Tests (Playwright) | Hoàn thành |
  | CI/CD Pipeline | Cấu hình GitHub Actions tự động kiểm thử và triển khai GitHub Pages | Hoàn thành |

## 15. Quy trình làm việc trên Git

Dự án áp dụng quy trình quản lý mã nguồn Git chặt chẽ cho cá nhân:

1. **Nhánh** `main`**:** Lưu trữ phiên bản ổn định nhất của ứng dụng dùng cho Production (tự động trigger CD deploy lên GitHub Pages).
2. **Nhánh** `develop`**:** Nhánh tích hợp và phát triển chính. Các tính năng mới được hoàn thiện trên các nhánh tính năng trước khi gộp về `develop`.
3. **Nhánh tính năng (**`feature/feature-name`**):** Phát triển các mô-đun chức năng độc lập (ví dụ: `feature/invoice`, `feature/backup`).
4. **Quy trình Kiểm định:** Trước khi merge code từ `feature` sang `develop` và `main`, toàn bộ bộ test (Unit test và Playwright E2E) phải chạy thành công 100% cả ở môi trường local và trên GitHub Actions CI.
...
Trong suốt quá trình thực hiện dự án, tác giả đã kết hợp sử dụng Trợ lý lập trình AI **Antigravity (Google DeepMind)** để tăng hiệu suất phát triển theo chuẩn yêu cầu §24.2:
