...
* **Thành viên 1 (`[NHẬP]`):** `[NHẬP]`
* **Thành viên 2 (`[NHẬP]`):** `[NHẬP]`
* **Bảng phân công nhiệm vụ cụ thể:**
  | Thành viên | Nhiệm vụ | Trạng thái |
  | :--- | :--- | :--- |
  | `[NHẬP]` | Thiết kế UI/UX, Trang Dashboard & Thống kê | Hoàn thành |
  | `[NHẬP]` | Phát triển Logic nghiệp vụ (Business) & Services CRUD | Hoàn thành |
  | `[NHẬP]` | Viết Unit Test (Vitest) & E2E Test (Playwright) | Hoàn thành |
  | `[NHẬP]` | Cấu hình CI/CD Pipelines & Deploy sản phẩm | Hoàn thành |
...
Trong suốt quá trình thực hiện dự án, nhóm đã kết hợp sử dụng Trợ lý lập trình AI **Antigravity (Google DeepMind)** để tăng hiệu suất phát triển:
* **Tối ưu mã nguồn & Refactoring:** AI hỗ trợ cấu trúc lại các hàm tính tiền dịch vụ phức tạp, chuyển đổi router sạch sang dạng lai kép tương thích linh hoạt trên GitHub Pages.
* **Xây dựng bộ kiểm thử tự động:** AI đề xuất cấu trúc, giả lập dữ liệu độc lập và viết nhanh 168 unit tests bằng Vitest bao phủ 100% logic kinh doanh.
* **Thiết kế kịch bản E2E:** Hỗ trợ viết các luồng test E2E đồng bộ trên Playwright (xử lý chính xác các dropdown tìm kiếm tùy biến và phát hiện reload trang không sử dụng hard-coded timeout).
